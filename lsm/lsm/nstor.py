...
class NexentaStor(INfs, IStorageAreaNetwork):
    ...
    def _to_bytes(self, size):
        ...
        if size.lower().endswith('e'):
            return int(float(size[:-1]) * 1024*1024*1024*1024*1024*1024)
        return size
    ...
    def fs_snapshots(self, fs, flags = 0):
        ...
        for snapshot in snapshot_list:
            snapshot_info = self._ns_request('rest/nms',
                {"method": "get_child_props",
                 "object": "snapshot",
                 "params": [snapshot, "creation_seconds"]})
    ...
    def fs_snapshot_create(self, fs, snapshot_name, files, flags = 0):
        ...
        snapshot_info = self._ns_request('rest/nms',
            {"method": "get_child_props",
             "object": "snapshot",
             "params": [full_name, "creation_seconds"]})
...
#        c.set(Capabilities.VOLUME_OFFLINE)
    ...
    def fs_clone(self, src_fs, dest_fs_name, snapshot=None, flags = 0):
        ...
        fs = FileSystem(dest_fs_name, dest_fs_name,
            self._to_bytes(pool_info['size']),
            self._to_bytes(pool_info['available']), pool_id, dest_fs_name)
        return None, fs
    ...
    def export_auth(self, flags = 0):
        ...
        """
        result = self._ns_request('rest/nms', {"method": "get_share_confopts",
                                  "object": "netstorsvc",
    ...
    def exports(self, flags = 0):
        ...
        """
        list = self._ns_request('rest/nms', {"method": "get_shared_folders",
            "object": "netstorsvc",
            "params": ['svc:/network/nfs/server:default', '']})
        exports = []
        for e in list:
            opts = self._ns_request('rest/nms',
                ...
                "object": "netstorsvc",
                "params": ['svc:/network/nfs/server:default', e]})
            exports.append(NfsExport(md5(opts['name']),
                e, opts['name'], opts['auth_type'], opts['root'],
                opts['read_write'], opts['read_only'], 'N/A', 'N/A',
                opts['extra_options']))
    ...
    def export_fs(self, fs_id, export_path, root_list, rw_list, ro_list,
        ...
        """
        id = md5(export_path)
        dict = {'auth_type': 'sys', 'anonymous': 'false'}
        if ro_list:
            dict['read_only'] = ','.join(ro_list)
        if rw_list:
            dict['read_write'] = ','.join(rw_list)
        if anon_uid or anon_gid:
            dict['anonymous'] = 'true'
        if root_list:
            dict['root'] = ','.join(root_list)
        if auth_type:
            dict['auth_type'] = str(auth_type)
        if '*' in rw_list:
            dict['anonymous'] = 'true'
        if options:
            dict['extra_options'] = str(options)
        result = self._ns_request('rest/nms',
            {"method": "share_folder",
            "object": "netstorsvc",
            "params": ['svc:/network/nfs/server:default', fs_id, dict]})
        return NfsExport(id, fs_id, export_path, auth_type,
...
#             "params": ['']})
    ...
    def volume_create(self, pool, volume_name, size_bytes, provisioning,
        ...
        new_volume = Volume(name, name, '', 8192, size_bytes / 8192, '', '',
            pool.id) #FIXhttp://192.168.0.1/st_wlan.phpME repace with list request
...
#                                                     False, rec]})
        ...
        try:
            ret = self._ns_request('rest/nms',
                {"method": "create_initiator",
                 "object": "iscsitarget",
                 "params": [initiator.name , {'initiatorchapuser': user,
                                         'initiatorchapsecret': password}]})
        except:
            ret = self._ns_request('rest/nms',
    ...
    def _access_group_grant(self, group_name, volume_name, access):
        ret = self._ns_request('rest/nms',
            {"method": "add_lun_mapping_entry",
             "object": "scsidisk",
             "params": [volume_name, {'host_group': group_name}]})
        return
    ...
    def access_group_list(self, flags = 0):
        ...
        """
        list = self._ns_request('rest/nms',
    ...
    def _initiator_human_type(self, id):
        ...
        for ag in self.access_group_list():
            if initiator_id in ag.initiators:
                raise LsmError(ErrorNumber.EXISTS_INITIATOR,
                    "%s is already part of %s access group" % ( initiator_id,
                    ag.name))
        self._ns_request('rest/nms',
