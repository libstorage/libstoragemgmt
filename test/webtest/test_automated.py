...
def run_test(cmdline, output_dir, sys_id, uri, password):
    ...
    with open(fn + ".ec", "w") as error_file:
        error_file.write(yaml.dump(dict(ec=str(ec), error_file=fn + ".error", uri=uri)))
        error_file.flush()


if __name__ == "__main__":
    time_limit_seconds = int(os.getenv("LSM_TEST_TMO_SECS", 90 * 60))  # 90 minutes

    if len(sys.argv) != 4:
        print(
            "Syntax: %s <array_file> <plugin unit test> <output directory>"
            % (sys.argv[0])
        )
        sys.exit(1)
    else:
        ...
        for system in arrays_to_test:
            ...
            p = Process(
                target=run_test,
                args=(sys.argv[2], sys.argv[3], system_id, u, credentials),
            )
            p.name = system_id
            p.start()
        ...
        start = time.time()
        print(
            "Test run started at: %s, time limit is %s minutes"
        ...
        while len(process_list) > 0:
            for p in process_list:
                ...
                if not p.is_alive():
                    print(
                        "%s exited with %s at %s (runtime %s seconds)"
                        % (
                            p.name,
                            str(p.exitcode),
                            time.strftime("%c"),
            ...
            if (current - start) >= time_limit_seconds:
                ...
                sys.stdout.flush()
                for p in process_list:
                    print("Terminating process %s, name %s" % (str(p.pid), p.name))
                    sys.stdout.flush()
