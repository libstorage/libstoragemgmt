...
(OP_SYS, OP_POOL, OP_VOL, OP_INIT, OP_FS, OP_EXPORTS, OP_SS) = ('SYSTEMS', 'POOLS',
                                                        'VOLUMES',
                                                        'INITIATORS', 'FS',
                                                        'EXPORTS', 'SNAPSHOTS')
...
def rs(l):
    ...
    """
    return 'lsm_' + ''.join(random.choice(string.ascii_uppercase) for x in range(l))
...
def access_group_add_init(group, initiator):
    call([cmd, '--access-group-add', group, '--id', initiator, '--type', 'ISCSI'])
...
def access_group_grant(group, volume_id):
    call([cmd, '--access-grant-group', group, '--volume', volume_id, '--access', 'RW' ])
...
def clone_fs(fs_id):
    out = call([cmd, '--clone-fs', fs_id, '--name', 'cloned_'+rs(8) , '-t'+sep ])[1]
    r = parse(out)
    return r[0][ID]
...
def initiator_grant(iqn, vol_id):
    #initiator_grant(self, initiator_id, initiator_type, volume, access,
     #   flags = 0):
    call( [cmd, '--access-grant', iqn, '--type', 'ISCSI', '--volume', vol_id, '--access', 'RW'] )
...
if __name__ == "__main__":
    ...
    parser.add_option("-p", "--pool", action="store", dest="pool_name",
        default = 'lsm_test_aggr', help="pool name to use for testing")
