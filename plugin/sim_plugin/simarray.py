    ...
    def wrapper(*args, **kargs):
        try:
            ...
            if str(sql_error) == "database is locked":
                raise LsmError(
                    ErrorNumber.TIMEOUT, "Timeout to require lock on state file"
                )
            raise LsmError(
            ...
            if type(args[0]) is SimArray and hasattr(args[0], "bs_obj"):
                args[0].bs_obj.trans_rollback()
            raise LsmError(
                ErrorNumber.PLUGIN_BUG, "Got unexpected error: %s" % str(base_error)
            )
...
class PoolRAID(object):
    ...
    def data_disk_count(raid_type, disk_count):
        ...
        if raid_type not in list(PoolRAID._RAID_DISK_CHK.keys()):
            raise LsmError(
                ErrorNumber.PLUGIN_BUG,
                "data_disk_count(): Got unsupported raid type(%d)" % raid_type,
            )
...
class BackStore(object):
    ...
    def _check_version(self):
        ...
        if len(sim_syss) == 0 or not sim_syss[0]:
            ...
            if (
                "version" in sim_syss[0].keys()
                and sim_syss[0]["version"] == BackStore.VERSION_SIGNATURE
            ):
                return True
    ...
    def sim_job_create(self, job_data_type=None, data_id=None):
        ...
        self._data_add(
            "jobs",
            {
                "duration": os.getenv("LSM_SIM_TIME", BackStore.JOB_DEFAULT_DURATION),
                "timestamp": time.time(),
    ...
    def sim_job_status(self, sim_job_id):
        ...
        """
        sim_job = self._data_find("jobs", "id=%s" % sim_job_id, flag_unique=True)
        if sim_job is None:
            raise LsmError(ErrorNumber.NOT_FOUND_JOB, "Job not found")

        if sim_job["duration"] != 0:
            progress = int(
                (time.time() - float(sim_job["timestamp"])) / sim_job["duration"] * 100
            )
        else:
            progress = 100
    ...
    def lsm_disk_ids_of_pool(self, sim_pool_id):
        return list(
            d["lsm_disk_id"]
            for d in self._data_find("disks_view", 'owner_pool_id="%s"' % sim_pool_id)
        )
    ...
    def sim_pool_create_from_disk(
        ...
        if raid_type == Volume.RAID_TYPE_RAID1 or raid_type == Volume.RAID_TYPE_JBOD:
        ...
        data_disk_count = PoolRAID.data_disk_count(raid_type, len(sim_disk_ids))

        # update disk owner
        sim_pool_id = self.lastrowid
        for sim_disk_id in sim_disk_ids[:data_disk_count]:
            self._data_update("disks", sim_disk_id, "owner_pool_id", sim_pool_id)
            self._data_update("disks", sim_disk_id, "role", "DATA")

        for sim_disk_id in sim_disk_ids[data_disk_count:]:
            self._data_update("disks", sim_disk_id, "owner_pool_id", sim_pool_id)
            self._data_update("disks", sim_disk_id, "role", "PARITY")

        return sim_pool_id

    def sim_pool_create_sub_pool(
        self, name, parent_pool_id, size, element_type, unsupported_actions=0
    ):
        self._data_add(
            "pools",
            {
    ...
    def sim_pool_disks_count(self, sim_pool_id):
        return self._sql_exec(
            "SELECT COUNT(id) FROM disks WHERE owner_pool_id=%s;" % sim_pool_id
        )[0][0]

    def sim_pool_data_disks_count(self, sim_pool_id=None):
        return self._sql_exec(
            "SELECT COUNT(id) FROM disks WHERE "
            "owner_pool_id=%s and role='DATA';" % sim_pool_id
        )[0][0]
    ...
    def sim_vols(self, sim_ag_id=None):
        ...
        """
        if sim_ag_id:
            return self._data_find("volumes_by_ag_view", "ag_id=%s" % sim_ag_id)
        else:
            return self._get_table("volumes_view")

    def _sim_data_of_id(self, table_name, data_id, lsm_error_no, data_name):
        sim_data = self._data_find(table_name, "id=%s" % data_id, flag_unique=True)
        if sim_data is None:
            if lsm_error_no:
                raise LsmError(lsm_error_no, "%s not found" % data_name)
            else:
                return None
        return sim_data
    ...
    def sim_vol_of_id(self, sim_vol_id):
        ...
        """
        return self._sim_data_of_id(
            "volumes_view", sim_vol_id, ErrorNumber.NOT_FOUND_VOLUME, "Volume"
        )
    ...
    def _check_pool_free_space(self, sim_pool_id, size_bytes):
        ...
        if sim_pool["free_space"] < size_bytes:
            raise LsmError(ErrorNumber.NOT_ENOUGH_SPACE, "Insufficient space in pool")

    @staticmethod
    def _block_rounding(size_bytes):
        return (
            (size_bytes + BackStore.BLK_SIZE - 1)
            // BackStore.BLK_SIZE
            * BackStore.BLK_SIZE
        )
    ...
    def sim_vol_delete(self, sim_vol_id):
        ...
        if self._sim_ag_ids_of_masked_vol(sim_vol_id):
            raise LsmError(ErrorNumber.IS_MASKED, "Volume is masked to access group")
        ...
        if sim_vol["is_hw_raid_vol"]:
            ...
            for d in self._data_find(
                "disks_view", 'owner_pool_id="%s"' % sim_vol["pool_id"]
            ):
    ...
    def sim_vol_mask(self, sim_vol_id, sim_ag_id):
        ...
        exist_mask = self._data_find(
            "vol_masks", 'ag_id="%s" AND vol_id="%s"' % (sim_ag_id, sim_vol_id)
        )
        if exist_mask:
            raise LsmError(
        ...
        self._data_add("vol_masks", {"ag_id": sim_ag_id, "vol_id": sim_vol_id})
    ...
    def _sim_vol_ids_of_masked_ag(self, sim_ag_id):
        return list(
            m["vol_id"] for m in self._data_find("vol_masks", 'ag_id="%s"' % sim_ag_id)
        )

    def _sim_ag_ids_of_masked_vol(self, sim_vol_id):
        return list(
            m["ag_id"] for m in self._data_find("vol_masks", 'vol_id="%s"' % sim_vol_id)
        )
    ...
    def sim_vol_resize(self, sim_vol_id, new_size_bytes):
        ...
        if sim_vol["total_space"] == new_size_bytes:
            raise LsmError(
                ErrorNumber.NO_STATE_CHANGE, "Volume size is identical to requested"
            )
        ...
        if increment > 0:
            ...
            raise LsmError(
                ErrorNumber.NO_SUPPORT, "Requested pool does not allow volume size grow"
            )

        # TODO(Gris Ge): If a volume is in a replication relationship, resize
        #                should be handled properly.
        self._data_update("volumes", sim_vol_id, "total_space", new_size_bytes)
        self._data_update("volumes", sim_vol_id, "consumed_size", new_size_bytes)
    ...
    def dst_sim_vol_ids_of_src(self, src_sim_vol_id):
        ...
        return list(
            d["dst_vol_id"]
            for d in self._data_find("vol_reps", 'src_vol_id="%s"' % src_sim_vol_id)
        )

    def sim_vol_replica(
        self, src_sim_vol_id, dst_sim_vol_id, rep_type, blk_ranges=None
    ):
        self.sim_vol_of_id(src_sim_vol_id)
        ...
        cur_src_sim_vol_ids = list(
            r["src_vol_id"]
            for r in self._data_find("vol_reps", 'dst_vol_id="%s"' % dst_sim_vol_id)
        )
    ...
    def sim_vol_src_replica_break(self, src_sim_vol_id):
        ...
        self._data_delete("vol_reps", 'src_vol_id="%s"' % src_sim_vol_id)
    ...
    def sim_vol_state_change(self, sim_vol_id, new_admin_state):
        ...
        self._data_update("volumes", sim_vol_id, "admin_state", new_admin_state)
    ...
    def _sim_ag_format(sim_ag):
        ...
        """
        sim_ag["init_ids"] = sim_ag["init_ids_str"].split(BackStore._LIST_SPLITTER)
        del sim_ag["init_ids_str"]
        return sim_ag

    def sim_ags(self, sim_vol_id=None):
        if sim_vol_id:
            sim_ags = self._data_find("ags_by_vol_view", "vol_id=%s" % sim_vol_id)
        else:
    ...
    def _sim_init_create(self, init_type, init_id, sim_ag_id):
        try:
            self._data_add(
                "inits",
                {"id": init_id, "init_type": init_type, "owner_ag_id": sim_ag_id},
            )
        except sqlite3.IntegrityError as sql_error:
            raise LsmError(
                ErrorNumber.EXISTS_INITIATOR,
                "Initiator '%s' is already in use by other access group" % init_id,
            )

    def iscsi_chap_auth_set(self, init_id, in_user, in_pass, out_user, out_pass):
    ...
    def sim_ag_create(self, name, init_type, init_id):
        try:
            ...
            raise LsmError(
                ErrorNumber.NAME_CONFLICT,
                "Name '%s' is already in use by other access group" % name,
            )
    ...
    def sim_ag_delete(self, sim_ag_id):
        ...
        if self._sim_vol_ids_of_masked_ag(sim_ag_id):
            raise LsmError(ErrorNumber.IS_MASKED, "Access group has volume masked to")
    ...
    def sim_ag_init_add(self, sim_ag_id, init_id, init_type):
        ...
        if init_id in sim_ag["init_ids"]:
            raise LsmError(
                ErrorNumber.NO_STATE_CHANGE, "Initiator already in access group"
            )

        if (
            init_type != AccessGroup.INIT_TYPE_ISCSI_IQN
            and init_type != AccessGroup.INIT_TYPE_WWPN
        ):
            raise LsmError(
                ErrorNumber.NO_SUPPORT, "Only support iSCSI IQN and WWPN initiator type"
            )
    ...
    def sim_ag_init_delete(self, sim_ag_id, init_id):
        ...
        if init_id not in sim_ag["init_ids"]:
            raise LsmError(
                ErrorNumber.NO_STATE_CHANGE, "Initiator is not in defined access group"
            )
    ...
    def sim_ag_of_id(self, sim_ag_id):
        sim_ag = self._sim_data_of_id(
            "ags_view", sim_ag_id, ErrorNumber.NOT_FOUND_ACCESS_GROUP, "Access Group"
        )
    ...
    def sim_fs_of_id(self, sim_fs_id, raise_error=True):
        ...
        return self._sim_data_of_id("fss_view", sim_fs_id, lsm_error_no, "File System")
    ...
    def sim_fs_resize(self, sim_fs_id, new_size_bytes):
        ...
        if (
            new_size_bytes > sim_fs["total_space"]
            and sim_pool["free_space"] < new_size_bytes - sim_fs["total_space"]
        ):
            raise LsmError(ErrorNumber.NOT_ENOUGH_SPACE, "Insufficient space in pool")

        self._data_update("fss", sim_fs_id, "total_space", new_size_bytes)
        self._data_update("fss", sim_fs_id, "consumed_size", new_size_bytes)
        self._data_update("fss", sim_fs_id, "free_space", new_size_bytes)
    ...
    def sim_fs_snap_restore(
        self, sim_fs_id, sim_fs_snap_id, files, restore_files, flag_all_files
    ):
    ...
    def sim_fs_file_clone(self, sim_fs_id, src_fs_name, dst_fs_name, sim_fs_snap_id):
    ...
    def clone_dst_sim_fs_ids_of_src(self, src_sim_fs_id):
        ...
        return list(
            d["dst_fs_id"]
            for d in self._data_find("fs_clones", 'src_fs_id="%s"' % src_sim_fs_id)
        )
    ...
    def _sim_exp_format(self, sim_exp):
        for key_name in ["root_hosts", "rw_hosts", "ro_hosts"]:
            ...
            if sim_exp[table_name]:
                sim_exp[key_name] = sim_exp[table_name].split(BackStore._LIST_SPLITTER)
            else:
                sim_exp[key_name] = []
    ...
    def sim_exps(self):
        return list(self._sim_exp_format(e) for e in self._get_table("exps_view"))

    def sim_exp_of_id(self, sim_exp_id):
        return self._sim_exp_format(
            self._sim_data_of_id(
                "exps_view", sim_exp_id, ErrorNumber.NOT_FOUND_NFS_EXPORT, "NFS Export"
            )
        )
...
class SimArray(object):
    ...
    def _sim_vol_id_of(vol_id):
        return SimArray._lsm_id_to_sim_id(
            vol_id, LsmError(ErrorNumber.NOT_FOUND_VOLUME, "Volume not found")
        )

    @staticmethod
    def _sim_fs_id_of(fs_id):
        return SimArray._lsm_id_to_sim_id(
            fs_id, LsmError(ErrorNumber.NOT_FOUND_FS, "File system not found")
        )
    ...
    def _sim_fs_snap_id_of(snap_id):
        return SimArray._lsm_id_to_sim_id(
            snap_id,
            LsmError(ErrorNumber.NOT_FOUND_FS_SS, "File system snapshot not found"),
        )
    ...
    def _sim_exp_id_of(exp_id):
        return SimArray._lsm_id_to_sim_id(
            exp_id,
            LsmError(ErrorNumber.NOT_FOUND_NFS_EXPORT, "File system export not found"),
        )
    ...
    def _job_create(self, data_type=None, sim_data_id=None):
        sim_job_id = self.bs_obj.sim_job_create(data_type, sim_data_id)
    ...
    def job_status(self, job_id, flags=0):
        ...
        (progress, data_type, sim_data) = self.bs_obj.sim_job_status(sim_job_id)
    ...
    def systems(self):
        return list(
            SimArray._sim_sys_2_lsm(sim_sys) for sim_sys in self.bs_obj.sim_syss()
        )
    ...
    def system_read_cache_pct_update(self, system, read_pct, flags=0):
        ...
        self.bs_obj.trans_begin()
        self.bs_obj._data_update(
            "systems", BackStore.SYS_ID, "read_cache_pct", read_pct
        )
        self.bs_obj.trans_commit()
    ...
    @_handle_errors
    def volumes(self):
        return list(SimArray._sim_vol_2_lsm(v) for v in self.bs_obj.sim_vols())
    ...
    def pools(self, flags=0):
        ...
        return list(SimArray._sim_pool_2_lsm(sim_pool) for sim_pool in sim_pools)
    ...
    def _sim_disk_2_lsm(sim_disk):
        ...
        return Disk(
            ...
            disk_status,
            BackStore.SYS_ID,
            _vpd83=sim_disk["vpd83"],
            _location=sim_disk["location"],
            _rpm=sim_disk["rpm"],
            _link_type=sim_disk["link_type"],
        )

    @_handle_errors
    def disks(self):
        return list(
            SimArray._sim_disk_2_lsm(sim_disk) for sim_disk in self.bs_obj.sim_disks()
        )
    ...
    def volume_create(
        ...
        new_sim_vol_id = self.bs_obj.sim_vol_create(
            vol_name,
            size_bytes,
            SimArray._sim_pool_id_of(pool_id),
        ...
        job_id = self._job_create(BackStore.JOB_DATA_TYPE_VOL, new_sim_vol_id)
        self.bs_obj.trans_commit()
    ...
    def volume_resize(self, vol_id, new_size_bytes, flags=0):
        ...
        job_id = self._job_create(BackStore.JOB_DATA_TYPE_VOL, sim_vol_id)
        self.bs_obj.trans_commit()
    ...
    def volume_replicate(
        self, dst_pool_id, rep_type, src_vol_id, new_vol_name, flags=0
    ):
        self.bs_obj.trans_begin()
        ...
        job_id = self._job_create(BackStore.JOB_DATA_TYPE_VOL, dst_sim_vol_id)
        self.bs_obj.trans_commit()
    ...
    @_handle_errors
    def volume_replicate_range(self, rep_type, src_vol_id, dst_vol_id, ranges, flags=0):
        self.bs_obj.trans_begin()
    ...
    def volume_child_dependency_rm(self, vol_id, flags=0):
        ...
        self.bs_obj.sim_vol_src_replica_break(SimArray._sim_vol_id_of(vol_id))
    ...
    @_handle_errors
    def fs_create(self, pool_id, fs_name, size_bytes, flags=0, _internal_use=False):
        ...
        job_id = self._job_create(BackStore.JOB_DATA_TYPE_FS, new_sim_fs_id)
        self.bs_obj.trans_commit()
    ...
    def fs_clone(self, src_fs_id, dst_fs_name, snap_id, flags=0):
        ...
        dst_sim_fs_id = self.fs_create(
            pool_id, dst_fs_name, src_sim_fs["total_space"], _internal_use=True
        )

        self.bs_obj.sim_fs_clone(src_sim_fs_id, dst_sim_fs_id, sim_fs_snap_id)

        job_id = self._job_create(BackStore.JOB_DATA_TYPE_FS, dst_sim_fs_id)
        self.bs_obj.trans_commit()
    ...
    def fs_file_clone(self, fs_id, src_fs_name, dst_fs_name, snap_id, flags=0):
        ...
        self.bs_obj.sim_fs_file_clone(
            SimArray._sim_fs_id_of(fs_id), src_fs_name, dst_fs_name, sim_fs_snap_id
        )
    ...
    def _sim_fs_snap_2_lsm(sim_fs_snap):
        return FsSnapshot(
            sim_fs_snap["lsm_fs_snap_id"], sim_fs_snap["name"], sim_fs_snap["timestamp"]
        )
    ...
    def fs_snapshots(self, fs_id, flags=0):
        return list(
            ...
            for s in self.bs_obj.sim_fs_snaps(SimArray._sim_fs_id_of(fs_id))
        )
    ...
    def fs_snapshot_create(self, fs_id, snap_name, flags=0):
        ...
        job_id = self._job_create(BackStore.JOB_DATA_TYPE_FS_SNAP, sim_fs_snap_id)
        self.bs_obj.trans_commit()
        return job_id, None
    ...
    def fs_snapshot_delete(self, fs_id, snap_id, flags=0):
        ...
        self.bs_obj.sim_fs_snap_delete(
            SimArray._sim_fs_snap_id_of(snap_id), SimArray._sim_fs_id_of(fs_id)
        )
        job_id = self._job_create()
        self.bs_obj.trans_commit()
        return job_id
    ...
    def fs_snapshot_restore(
        self, fs_id, snap_id, files, restore_files, flag_all_files, flags
    ):
        self.bs_obj.trans_begin()
        sim_fs_snap_id = None
        if snap_id:
    ...
    def fs_child_dependency(self, fs_id, files, flags=0, _internal_use=False):
        ...
        if _internal_use is False:
            self.bs_obj.trans_begin()
        if (
            self.bs_obj.clone_dst_sim_fs_ids_of_src(sim_fs_id) == []
            and self.bs_obj.sim_fs_snaps(sim_fs_id) == []
        ):
            if _internal_use is False:
                self.bs_obj.trans_rollback()
            return False
    ...
    @_handle_errors
    def access_group_initiator_delete(self, ag_id, init_id, init_type, flags=0):
        sim_ag_id = SimArray._sim_ag_id_of(ag_id)
        self.bs_obj.trans_begin()
    ...
    def volume_mask(self, ag_id, vol_id, flags=0):
        ...
        self.bs_obj.sim_vol_mask(
            SimArray._sim_vol_id_of(vol_id), SimArray._sim_ag_id_of(ag_id)
        )
        self.bs_obj.trans_commit()
        return None
    ...
    def volumes_accessible_by_access_group(self, ag_id, flags=0):
        ...
        sim_vols = self.bs_obj.sim_vols(sim_ag_id=SimArray._sim_ag_id_of(ag_id))
    ...
    def access_groups_granted_to_volume(self, vol_id, flags=0):
        self.bs_obj.trans_begin()
        sim_ags = self.bs_obj.sim_ags(sim_vol_id=SimArray._sim_vol_id_of(vol_id))
        self.bs_obj.trans_rollback()
        return [SimArray._sim_ag_2_lsm(a) for a in sim_ags]

    @_handle_errors
    def iscsi_chap_auth(self, init_id, in_user, in_pass, out_user, out_pass, flags=0):
        self.bs_obj.trans_begin()
        self.bs_obj.iscsi_chap_auth_set(init_id, in_user, in_pass, out_user, out_pass)
        self.bs_obj.trans_commit()
        return None
    ...
    def _sim_tgt_2_lsm(sim_tgt):
        return TargetPort(
            sim_tgt["lsm_tgt_id"],
            sim_tgt["port_type"],
            sim_tgt["service_address"],
            sim_tgt["network_address"],
            sim_tgt["physical_address"],
    ...
    def volume_raid_info(self, lsm_vol):
        sim_pool = self.bs_obj.sim_pool_of_id(
            SimArray._lsm_id_to_sim_id(
                lsm_vol.pool_id, LsmError(ErrorNumber.NOT_FOUND_POOL, "Pool not found")
            )
        )
        ...
        if raid_type == Volume.RAID_TYPE_UNKNOWN or raid_type == Volume.RAID_TYPE_OTHER:
    ...
    def pool_member_info(self, lsm_pool):
        sim_pool = self.bs_obj.sim_pool_of_id(
            SimArray._lsm_id_to_sim_id(
                lsm_pool.id, LsmError(ErrorNumber.NOT_FOUND_POOL, "Pool not found")
            )
        )
    ...
    def volume_raid_create_cap_get(self, system):
        if system.id != BackStore.SYS_ID:
            raise LsmError(ErrorNumber.NOT_FOUND_SYSTEM, "System not found")
        return (BackStore.SUPPORTED_VCR_RAID_TYPES, BackStore.SUPPORTED_VCR_STRIP_SIZES)
    ...
    def volume_raid_create(self, name, raid_type, disks, strip_size):
        ...
        sim_disk_ids = [
            SimArray._lsm_id_to_sim_id(
                d.id, LsmError(ErrorNumber.NOT_FOUND_DISK, "Disk not found")
            )
            for d in disks
        ]
        ...
        sim_vol_id = self.volume_create(
            ...
            sim_pool["lsm_pool_id"],
            name,
            sim_pool["free_space"] - 1024,
            Volume.PROVISION_FULL,
            _internal_use=True,
            _is_hw_raid_vol=1,
        )
    ...
    def volume_ident_led_on(self, volume, flags=0):
        sim_volume_id = SimArray._lsm_id_to_sim_id(
            volume.id, LsmError(ErrorNumber.NOT_FOUND_VOLUME, "Volume not found")
        )
    ...
    def volume_cache_info(self, lsm_vol):
        sim_vol = self.bs_obj.sim_vol_of_id(
            SimArray._lsm_id_to_sim_id(
                lsm_vol.id, LsmError(ErrorNumber.NOT_FOUND_VOLUME, "Volume not found")
            )
        )
        ...
        elif sim_vol["write_cache_policy"] == Volume.WRITE_CACHE_POLICY_WRITE_BACK:
            write_cache_status = Volume.WRITE_CACHE_STATUS_WRITE_BACK
        ...
        return [
            sim_vol["write_cache_policy"],
            write_cache_status,
            sim_vol["read_cache_policy"],
            read_cache_status,
