...
class SimPlugin(INfs, IStorageAreaNetwork):
    ...
    def plugin_register(self, uri, password, timeout, flags=0):
        ...
        if (
            "parameters" in qp
            and "statefile" in qp["parameters"]
    ...
    def capabilities(self, system, flags=0):
        ...
        rc.set(Capabilities.ACCESS_GROUPS_QUICK_SEARCH, Capabilities.UNSUPPORTED)
        rc.set(Capabilities.NFS_EXPORTS_QUICK_SEARCH, Capabilities.UNSUPPORTED)
        rc.set(Capabilities.TARGET_PORTS_QUICK_SEARCH, Capabilities.UNSUPPORTED)
        rc.set(
            Capabilities.VOLUME_PHYSICAL_DISK_CACHE_UPDATE_SYSTEM_LEVEL,
            Capabilities.UNSUPPORTED,
        )
        rc.set(
            Capabilities.VOLUME_WRITE_CACHE_POLICY_UPDATE_IMPACT_READ,
            Capabilities.UNSUPPORTED,
        )
        rc.set(
            Capabilities.VOLUME_WRITE_CACHE_POLICY_UPDATE_WB_IMPACT_OTHER,
            Capabilities.UNSUPPORTED,
        )
        rc.set(
            Capabilities.VOLUME_READ_CACHE_POLICY_UPDATE_IMPACT_WRITE,
            Capabilities.UNSUPPORTED,
        )
        return rc
    ...
    def pools(self, search_key=None, search_value=None, flags=0):
        ...
        return search_property(
            [SimPlugin._sim_data_2_lsm(p) for p in sim_pools], search_key, search_value
        )

    def volumes(self, search_key=None, search_value=None, flags=0):
        sim_vols = self.sim_array.volumes()
        return search_property(
            [SimPlugin._sim_data_2_lsm(v) for v in sim_vols], search_key, search_value
        )

    def disks(self, search_key=None, search_value=None, flags=0):
        sim_disks = self.sim_array.disks()
        return search_property(
            [SimPlugin._sim_data_2_lsm(d) for d in sim_disks], search_key, search_value
        )
    ...
    def volume_resize(self, volume, new_size_bytes, flags=0):
        sim_vol = self.sim_array.volume_resize(volume.id, new_size_bytes, flags)
        return SimPlugin._sim_data_2_lsm(sim_vol)
    ...
    def volume_replicate_range_block_size(self, system, flags=0):
        return self.sim_array.volume_replicate_range_block_size(system.id, flags)

    def volume_replicate_range(
        self, rep_type, volume_src, volume_dest, ranges, flags=0
    ):
    ...
    def access_groups(self, search_key=None, search_value=None, flags=0):
        ...
        return search_property(
            [SimPlugin._sim_data_2_lsm(a) for a in sim_ags], search_key, search_value
        )
    ...
    def volume_mask(self, access_group, volume, flags=0):
        return self.sim_array.volume_mask(access_group.id, volume.id, flags)

    def volume_unmask(self, access_group, volume, flags=0):
        return self.sim_array.volume_unmask(access_group.id, volume.id, flags)
    ...
    def access_groups_granted_to_volume(self, volume, flags=0):
        sim_vols = self.sim_array.access_groups_granted_to_volume(volume.id, flags)
        return [SimPlugin._sim_data_2_lsm(v) for v in sim_vols]

    def iscsi_chap_auth(
        self, init_id, in_user, in_password, out_user, out_password, flags=0
    ):
        if out_user and out_password and (in_user is None or in_password is None):
            raise LsmError(
                ErrorNumber.INVALID_ARGUMENT,
                "out_user and out_password only supported if " "inbound is supplied",
            )
    ...
    def fs(self, search_key=None, search_value=None, flags=0):
        ...
        return search_property(
            [SimPlugin._sim_data_2_lsm(f) for f in sim_fss], search_key, search_value
        )
    ...
    def fs_resize(self, fs, new_size_bytes, flags=0):
        sim_fs = self.sim_array.fs_resize(fs.id, new_size_bytes, flags)
        return SimPlugin._sim_data_2_lsm(sim_fs)

    def fs_clone(self, src_fs, dest_fs_name, snapshot=None, flags=0):
        if snapshot is None:
            return self.sim_array.fs_clone(src_fs.id, dest_fs_name, None, flags)
        return self.sim_array.fs_clone(src_fs.id, dest_fs_name, snapshot.id, flags)

    def fs_file_clone(self, fs, src_file_name, dest_file_name, snapshot=None, flags=0):
        if snapshot is None:
    ...
    def fs_snapshot_create(self, fs, snapshot_name, flags=0):
        return self.sim_array.fs_snapshot_create(fs.id, snapshot_name, flags)

    def fs_snapshot_delete(self, fs, snapshot, flags=0):
        return self.sim_array.fs_snapshot_delete(fs.id, snapshot.id, flags)

    def fs_snapshot_restore(
        self, fs, snapshot, files, restore_files, all_files=False, flags=0
    ):
    ...
    def exports(self, search_key=None, search_value=None, flags=0):
        ...
        return search_property(
            [SimPlugin._sim_data_2_lsm(e) for e in sim_exps], search_key, search_value
        )
    ...
    def target_ports(self, search_key=None, search_value=None, flags=0):
        ...
        return search_property(
            [SimPlugin._sim_data_2_lsm(t) for t in sim_tgts], search_key, search_value
        )
    ...
    def volume_raid_create(self, name, raid_type, disks, strip_size, flags=0):
        return self.sim_array.volume_raid_create(name, raid_type, disks, strip_size)
