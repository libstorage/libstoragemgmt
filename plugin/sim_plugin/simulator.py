...
from lsm import (uri_parse, VERSION, Capabilities, INfs, IStorageAreaNetwork,
                 search_property, Client, LsmError, ErrorNumber)
...
class SimPlugin(INfs, IStorageAreaNetwork):
    ...
    def pools(self, search_key=None, search_value=None, flags=0):
        ...
        return search_property(
            [SimPlugin._sim_data_2_lsm(p) for p in sim_pools], search_key,
            search_value)

    def volumes(self, search_key=None, search_value=None, flags=0):
        sim_vols = self.sim_array.volumes()
        return search_property(
            [SimPlugin._sim_data_2_lsm(v) for v in sim_vols], search_key,
            search_value)

    def disks(self, search_key=None, search_value=None, flags=0):
        sim_disks = self.sim_array.disks()
        return search_property(
            [SimPlugin._sim_data_2_lsm(d) for d in sim_disks], search_key,
            search_value)
    ...
    def volume_create(self,
                      ...
                      provisioning,
                      flags=0):
        sim_vol = self.sim_array.volume_create(pool.id, volume_name,
                                               size_bytes, provisioning, flags)
    ...
    def volume_resize(self, volume, new_size_bytes, flags=0):
        sim_vol = self.sim_array.volume_resize(volume.id, new_size_bytes,
                                               flags)
        return SimPlugin._sim_data_2_lsm(sim_vol)
    ...
    def volume_replicate(self, pool, rep_type, volume_src, name, flags=0):
        ...
        return self.sim_array.volume_replicate(dst_pool_id, rep_type,
                                               volume_src.id, name, flags)
    ...
    def volume_replicate_range(self,
                               ...
                               ranges,
                               flags=0):
        return self.sim_array.volume_replicate_range(rep_type, volume_src.id,
                                                     volume_dest.id, ranges,
                                                     flags)
    ...
    def access_groups(self, search_key=None, search_value=None, flags=0):
        ...
        return search_property([SimPlugin._sim_data_2_lsm(a) for a in sim_ags],
                               search_key, search_value)

    def access_group_create(self, name, init_id, init_type, system, flags=0):
        sim_ag = self.sim_array.access_group_create(name, init_id, init_type,
                                                    system.id, flags)
    ...
    def iscsi_chap_auth(self,
        ...
        if out_user and out_password and \
            ...
            raise LsmError(
                ...
                "inbound is supplied")
        return self.sim_array.iscsi_chap_auth(init_id, in_user, in_password,
                                              out_user, out_password, flags)
    ...
    def fs(self, search_key=None, search_value=None, flags=0):
        ...
        return search_property([SimPlugin._sim_data_2_lsm(f) for f in sim_fss],
                               search_key, search_value)
    ...
    def fs_clone(self, src_fs, dest_fs_name, snapshot=None, flags=0):
        if snapshot is None:
            return self.sim_array.fs_clone(src_fs.id, dest_fs_name, None,
                                           flags)
        return self.sim_array.fs_clone(src_fs.id, dest_fs_name, snapshot.id,
                                       flags)
    ...
    def fs_file_clone(self,
                      ...
                      flags=0):
        if snapshot is None:
            return self.sim_array.fs_file_clone(fs.id, src_file_name,
                                                dest_file_name, None, flags)

        return self.sim_array.fs_file_clone(fs.id, src_file_name,
                                            dest_file_name, snapshot.id, flags)
    ...
    def fs_snapshot_restore(self,
                            ...
                            all_files=False,
                            flags=0):
        return self.sim_array.fs_snapshot_restore(fs.id, snapshot.id, files,
    ...
    def exports(self, search_key=None, search_value=None, flags=0):
        ...
        return search_property(
            [SimPlugin._sim_data_2_lsm(e) for e in sim_exps], search_key,
            search_value)
    ...
    def export_fs(self,
                  ...
                  options,
                  flags=0):
        sim_exp = self.sim_array.fs_export(fs_id,
                                           export_path,
                                           root_list,
                                           rw_list,
                                           ro_list,
    ...
    def target_ports(self, search_key=None, search_value=None, flags=0):
        ...
        return search_property(
            [SimPlugin._sim_data_2_lsm(t) for t in sim_tgts], search_key,
            search_value)
    ...
    def volume_raid_create(self, name, raid_type, disks, strip_size, flags=0):
        return self.sim_array.volume_raid_create(name, raid_type, disks,
                                                 strip_size)
