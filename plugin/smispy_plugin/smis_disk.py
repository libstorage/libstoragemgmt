...
def _disk_id_of_cim_disk(cim_disk):
    if "SystemName" not in cim_disk or "DeviceID" not in cim_disk:
        raise LsmError(
...
def sys_id_of_cim_disk(cim_disk):
    if "SystemName" not in cim_disk:
        raise LsmError(
            ...
            "SystemName property: %s, %s" % (cim_disk.path, list(cim_disk.items())),
        )
...
def _disk_type_megaraid(cim_disk):
    if (
        cim_disk["MediaType"] == _MEGARAID_DISK_MEDIA_TYPE_SSD
        or cim_disk["MediaType"] == _MEGARAID_DISK_MEDIA_TYPE_SSD_FLASH
    ):
...
def cim_disk_to_lsm_disk(smis_common, cim_disk):
    ...
    cim_ext = _pri_cim_ext_of_cim_disk(
        smis_common, cim_disk.path, property_list=["BlockSize", "NumberOfBlocks"]
    )

    status = _disk_status_of_cim_disk(cim_disk)
    if smis_common.profile_check(
        SmisCommon.SNIA_SPARE_DISK_PROFILE,
    ...
    return Disk(disk_id, name, disk_type, block_size, num_of_block, status, sys_id)
