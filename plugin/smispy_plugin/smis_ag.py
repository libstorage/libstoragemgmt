...
def cim_spc_pros():
    ...
    """
    return ["DeviceID", "ElementName", "StorageID", "EMCAdapterRole", "SystemName"]
...
def cim_init_of_cim_spc_path(smis_common, cim_spc_path):
    ...
    if smis_common.profile_check(
        SmisCommon.SNIA_MASK_PROFILE, SmisCommon.SMIS_SPEC_VER_1_6, raise_error=False
    ):
        try:
...
def cim_spc_to_lsm_ag(smis_common, cim_spc, system_id):
    ...
    return AccessGroup(ag_id, ag_name, init_ids, init_type, system_id, plugin_data)
...
def lsm_ag_to_cim_spc_path(smis_common, lsm_ag):
    ...
    if smis_common.system_list and lsm_ag.system_id not in smis_common.system_list:
...
def cim_init_path_check_or_create(smis_common, system_id, init_id, init_type):
    ...
    cim_inits = smis_common.EnumerateInstances(
        "CIM_StorageHardwareID", PropertyList=_CIM_INIT_PROS
    )
    ...
    if init_type == AccessGroup.INIT_TYPE_WWPN:
        ...
        raise LsmError(
            ErrorNumber.PLUGIN_BUG,
            "cim_init_path_check_or_create(): Got invalid init_type: %d" % init_type,
        )
...
def cim_vols_masked_to_cim_spc_path(smis_common, cim_spc_path, property_list=None):
    """
