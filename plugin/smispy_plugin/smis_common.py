...
def _profile_register_load(wbem_conn):
    ...
    for namespace in namespace_check_list:
        try:
            ...
            if (
                e.args[0] == pywbem.CIM_ERR_NOT_SUPPORTED
                or e.args[0] == pywbem.CIM_ERR_INVALID_NAMESPACE
                or e.args[0] == pywbem.CIM_ERR_INVALID_CLASS
            ):
                pass
            else:
                raise
    ...
    if len(cim_rps) >= 1:
        for cim_rp in cim_rps:
            ...
            if profile_name in list(profile_dict.keys()):
                exist_ver_num = _profile_spec_ver_to_num(profile_dict[profile_name])
...
def _profile_spec_ver_to_num(spec_ver_str):
    ...
    if len(tmp_list) == 3:
        return (
            int(tmp_list[0]) * 10 ** 6 + int(tmp_list[1]) * 10 ** 3 + int(tmp_list[2])
        )
    return None
...
class SmisCommon(object):
    ...
    def __init__(
        ...
        if no_ssl_verify:
            try:
                self._wbem_conn = pywbem.WBEMConnection(
                    url, (username, password), namespace, no_verification=True
                )
            except TypeError:
        ...
        if namespace.lower() == SmisCommon._MEGARAID_NAMESPACE.lower():
            ...
            (self._profile_dict, self.root_blk_cim_rp) = _profile_register_load(
                self._wbem_conn
            )
    ...
    def profile_check(self, profile_name, spec_ver, raise_error=False):
        ...
        Returns:
            ...
            False
        """
        return _profile_check(self._profile_dict, profile_name, spec_ver, raise_error)
    ...
    def EnumerateInstances(self, ClassName, namespace=None, **params):
        ...
        return self._wbem_conn.EnumerateInstances(ClassName, namespace, **params)
    ...
    def EnumerateInstanceNames(self, ClassName, namespace=None, **params):
        if self._wbem_conn.default_namespace in dmtf.INTEROP_NAMESPACES:
            ...
            self._wbem_conn.default_namespace = self._vendor_namespace()
        params["LocalOnly"] = False
        return self._wbem_conn.EnumerateInstanceNames(ClassName, namespace, **params)
    ...
    def cim_job_of_job_id(self, job_id, property_list=None):
        ...
        if property_list is None:
            ...
            property_list = merge_list(property_list, SmisCommon.cim_job_pros())

        cim_jobs = self.EnumerateInstances(
            "CIM_ConcreteJob", PropertyList=property_list
        )
            ...
            if self._debug_path is not None:
                ...
                if os.path.isdir(self._debug_path):
                    ...
                    with open(debug_full, "w") as d:
                        d.write(
                            "REQUEST:\n%s\n\nREPLY:\n%s\n"
                            % (self._wbem_conn.last_request, self._wbem_conn.last_reply)
                        )
        except Exception:
    ...
    def invoke_method(
        ...
        try:
            ...
            if rc == SmisCommon.SNIA_INVOKE_OK:
                ...
                raise LsmError(
                    ErrorNumber.PLUGIN_BUG, "Error: %s rc= %s" % (cmd, str(rc))
                )
    ...
    def invoke_method_wait(
        ...
        try:
            if rc == SmisCommon.SNIA_INVOKE_OK:
                ...
                loop_counter = 0
                job_pros = ["JobState", "ErrorDescription", "OperationalStatus"]
                cim_xxxs_path = []
                while loop_counter <= SmisCommon._INVOKE_MAX_LOOP_COUNT:
                    cim_job = self.GetInstance(cim_job_path, PropertyList=job_pros)
                ...
                if len(cim_xxxs_path) == 1:
                    ...
                    raise LsmError(
                        ...
                        "%s, out: %s, job: %s"
                        % (cim_xxxs_path, list(out.items()), list(cim_job.items())),
                    )
            else:
                self._dump_wbem_xml(cmd)
                raise LsmError(
    ...
    def _cim_srv_of_sys_id(self, srv_name, sys_id, raise_error):
        ...
        try:
            cim_srvs = self.EnumerateInstances(srv_name, PropertyList=property_list)
        ...
        if raise_error:
            raise LsmError(
                ErrorNumber.NO_SUPPORT,
                "Cannot find any '%s' for requested system ID %s" % (srv_name, sys_id),
            )
        return None
    ...
    def cim_rs_of_sys_id(self, sys_id, raise_error=True):
        ...
        by DMTF CIM_Service.
        """
        return self._cim_srv_of_sys_id("CIM_ReplicationService", sys_id, raise_error)
    ...
    def cim_job_completed_ok(status):
        ...
        if len(op) > 1 and (
            ...
            or (op[0] == dmtf.OP_STATUS_COMPLETED and op[1] == dmtf.OP_STATUS_OK)
        ):
            rc = True
