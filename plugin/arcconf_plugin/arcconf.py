    ...
    def _wrapper(*args, **kwargs):
        try:
            ...
            if "No controllers detected" in exec_error.stdout:
                raise LsmError(
                    ErrorNumber.NOT_FOUND_SYSTEM, "No Controllers detected by arcconf."
                )
            else:
...
def _pool_status_of(arcconf_array):
    ...
    """
    if (
        arcconf_array[0]["Status"] == "OK"
        or arcconf_array[0]["Status"] == "Optimal"
        or arcconf_array[0]["Status"] == "Ready"
        or arcconf_array[0]["Status"] == "Online"
    ):
...
_LSM_RAID_TYPE_CONV = dict(
    list(
        zip(
            list(_ARCCONF_RAID_LEVEL_CONV.values()),
...
def _lsm_raid_type_to_arcconf(raid_type):
    try:
        ...
        raise LsmError(ErrorNumber.PLUGIN_BUG, "Not supported raid type %d" % raid_type)
...
class Arcconf(IPlugin):
    ...
    def plugin_register(self, uri, password, timeout, flags=Client.FLAG_RSVD):
        ...
        if not self._arcconf_bin:
            ...
            if not self._arcconf_bin:
                raise LsmError(
                    ErrorNumber.INVALID_ARGUMENT, "arcconf is not installed correctly"
                )
    ...
    def _get_arcconf_controllers_count(self):
        ...
        try:
            ...
            raise LsmError(
                ErrorNumber.NOT_FOUND_SYSTEM, "Failed to get controller count"
            )
        ...
        for decoded_json in getconfig_cntrls_info:
            ...
            if "Array" in decoded_json["Controller"]:
                ...
                for array in range(num_array):
                    ...
                    arcconf_array["unUsedSpace"] = array_infos[array]["unUsedSpace"]
                    arcconf_array["sys_id"] = sys_id
                    arcconf_array["ctrl_num"] = arcconf_ctrl_num
                    lsm_pools.append(Arcconf._arcconf_array_to_lsm_pool(arcconf_array))
                ...
                if "HardDrive" in channel_range:
                    ...
                    if hd_disks_num > 0:
                        for hd_disk in range(hd_disks_num):
                            arcconf_disk = channel_range["HardDrive"][hd_disk]
                            rc_lsm_disks.append(
    ...
    def volume_raid_create(
        ...
        for disk in disks:
            ...
            arcconf_disk_ids.append({"Channel": disk_channel, "Device": disk_device})
    ...
    def volume_delete(self, volume, flags=0):
        ...
        try:
            self._arcconf_exec(
                ["delete", ctrl_num, "logicaldrive", ld_id], flag_force=True
            )
        except ExecError:
    ...
    def volume_enable(self, volume, flags=Client.FLAG_RSVD):
        ...
        try:
            if volume_state == LOGICAL_DEVICE_OK:
                raise LsmError(
                    ErrorNumber.NO_STATE_CHANGE, "Volume is already in Optimal state!"
                )
            else:
                self._arcconf_exec(
            ...
            if volume_state == LOGICAL_DEVICE_OFFLINE:
                raise LsmError(
                    ErrorNumber.NO_STATE_CHANGE, "Volume state has not changed!"
                )
    ...
    def volume_ident_led_on(self, volume, flags=Client.FLAG_RSVD):
        ...
        try:
            ...
            raise LsmError(
                ErrorNumber.PLUGIN_BUG, "Volume-ident-led-on failed unexpectedly"
            )
    ...
    def volume_ident_led_off(self, volume, flags=Client.FLAG_RSVD):
        ...
        try:
            ...
            raise LsmError(
                ErrorNumber.NO_STATE_CHANGE, "Looks like none of the LEDs are blinking."
            )
