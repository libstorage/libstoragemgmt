...
static int _db_data_init(char *err_msg, sqlite3 *db)
{
    ...
    _good(_db_data_add(err_msg, db, _DB_TABLE_SYS,
                       "id", _SYS_ID,
                       "name", "LSM simulated storage plug-in",
                       "status", sys_status_str,
                       "status_info", "",
                       "read_cache_pct", _DEFAULT_SYS_READ_CACHE_PCT_STR,
    ...
    for (i = 0; i < 5; ++i) {
        ...
        _good(_db_data_add(err_msg, db, _DB_TABLE_DISKS,
                           ...
                           "total_space", _SIZE_512GIB_STR,
                           "disk_type", disk_type_str,
                           "status", disk_status_str,
                           "vpd83", _random_vpd(vpd_buff),
                           "rpm", disk_rpm_ssd_str,
                           "link_type", disk_link_type_str,
                           "location", location_str,
                           NULL),
              rc, out);
    ...
    _good(_db_data_add(err_msg, db, _DB_TABLE_TGTS,
                       "port_type", tgt_port_type_str,
    ...
    _good(_db_data_add(err_msg, db, _DB_TABLE_BATS,
                       "name", "Battery SIMB01, 8000 mAh, 05 March 2016",
                       "type", bat_type_str,
                       "status", bat_status_str,
                       NULL),
          rc, out);
    ...
    _good(_db_pool_create_from_disk(err_msg, db, "Pool 1", pool_1_disks,
                                    sizeof(pool_1_disks)/
                                    sizeof(pool_1_disks[0]),
                                    LSM_VOLUME_RAID_TYPE_RAID1,
    ...
    _good(_db_pool_create_from_disk(err_msg, db, "Pool 3", ssd_pool_disks,
                                    sizeof(ssd_pool_disks)/
                                    sizeof(ssd_pool_disks[0]),
                                    LSM_VOLUME_RAID_TYPE_RAID1,
                                    LSM_POOL_ELEMENT_TYPE_FS |
                                    LSM_POOL_ELEMENT_TYPE_VOLUME |
                                    LSM_POOL_ELEMENT_TYPE_DELTA,
    ...
    _good(_db_pool_create_from_disk(err_msg, db, "lsm_test_aggr",
                                    test_pool_disks,
                                    sizeof(test_pool_disks)/
                                    sizeof(test_pool_disks[0]),
                                    LSM_VOLUME_RAID_TYPE_RAID0,
                                    LSM_POOL_ELEMENT_TYPE_FS |
                                    LSM_POOL_ELEMENT_TYPE_VOLUME |
                                    LSM_POOL_ELEMENT_TYPE_DELTA,
                              ...
                              uint64_t *sim_pool_id, uint32_t strip_size)
{
    ...
    if (strip_size != LSM_VOLUME_VCR_STRIP_SIZE_DEFAULT) {
        found = false;
        for (j = 0;
             j < sizeof(_SUPPORTED_STRIP_SIZES) /
    ...
    if ((raid_type == LSM_VOLUME_RAID_TYPE_RAID1) ||
        ...
        if (strip_size != LSM_VOLUME_VCR_STRIP_SIZE_DEFAULT) {
            ...
            _lsm_err_msg_set(err_msg, "For RAID 1 and JBOD, strip size should "
    ...
    _snprintf_buff(err_msg, rc, out, strip_size_str, "%" PRIu32,
                   strip_size);
    ...
    rc = _db_data_add(err_msg, db, _DB_TABLE_POOLS,
                       "name", name,
                       "status", _POOL_STATUS_OK_STR,
                       "status_info", "",
                       "element_type", element_type_str,
                       "unsupported_actions", unsupported_actions_str,
                       "raid_type", raid_type_str,
                       "member_type", _POOL_MEMBER_TYPE_DISK_STR,
                                    ...
                                    uint64_t unsupported_actions)
{
    ...
    _good(_db_data_add(err_msg, db, _DB_TABLE_POOLS,
                       "name", name,
                       "status", _POOL_STATUS_OK_STR,
                       "status_info", "",
                       "element_type", element_type_str,
                       "raid_type", _VOLUME_RAID_TYPE_OTHER_STR,
                       "member_type", _POOL_MEMBER_TYPE_POOL_STR,
                       "parent_pool_id", parent_sim_pool_id_str,
             ...
             uint32_t timeout)
{
    ...
    if (db_rc != SQLITE_OK) {
        rc = LSM_ERR_INVALID_ARGUMENT;
        _lsm_err_msg_set(err_msg, "Failed to open SQLite database file '%s', "
                         "error %d: %s", db_file, db_rc,
                         sqlite3_errmsg(*db));
        goto out;
    }
    ...
    if (db_rc != SQLITE_OK) {
        rc = LSM_ERR_PLUGIN_BUG;
        _lsm_err_msg_set(err_msg, "Failed to set timeout %" PRIu32 ", "
                         "sqlite error %d %s", timeout, db_rc,
                         sqlite3_errmsg(*db));
        goto out;
    }
    ...
    if (db_check_rc == _DB_VERSION_CHECK_EMPTY) {
        ...
        rc = LSM_ERR_INVALID_ARGUMENT;
        _lsm_err_msg_set(err_msg, "Stored simulator state incompatible with "
                         "simulator, please move or delete %s", db_file);
        goto out;
    }
                 ...
                 struct _vector **vec)
{
    ...
    if (vec != NULL) {
        *vec = _vector_new(_VECTOR_NO_PRE_ALLOCATION);
        ...
        sql_rc = sqlite3_exec(db, cmd, _parse_sql_column,
                              *vec, &sql_err_msg);
    } else {
        sql_rc = sqlite3_exec(db, cmd, NULL /* callback func */,
                              NULL /* callback func first argument */,
                              &sql_err_msg);
    }
    if (sql_rc == SQLITE_BUSY) {
        ...
        rc = LSM_ERR_PLUGIN_BUG;
        if (sql_err_msg != NULL)
            _lsm_err_msg_set(err_msg, "SQLite error %d: %s, %s",
                             sql_rc, sqlite3_errmsg(db), sql_err_msg);
        else
            _lsm_err_msg_set(err_msg, "SQLite error %d: %s",
                             sql_rc, sqlite3_errmsg(db));
        goto out;
    }
...
{
    if (db != NULL)
        _db_sql_exec(NULL /* ignore error message */, db,
                     "ROLLBACK;", NULL /* don't parse output */);
}
...
int _db_data_add(char *err_msg, sqlite3 *db, const char *table_name, ...)
{
    ...
    while ((key_str != NULL) && (value_str != NULL)) {
        keys_printed += snprintf(keys_str + keys_printed,
                                 _BUFF_SIZE - keys_printed,
                                 "'%s', ", key_str) - 1;
        values_printed += snprintf(values_str + values_printed,
                                   _BUFF_SIZE - values_printed,
                                   "'%s', ", value_str) - 1;
                              ...
                              const char *table_name, const char *condition)
{
    ...
    if (snprintf(sql_cmd, _BUFF_SIZE, "DELETE FROM %s WHERE %s;",
                 table_name, condition) == _BUFF_SIZE) {
        _lsm_err_msg_set(err_msg, "BUG: _db_data_add(): Buff too small");
        return LSM_ERR_PLUGIN_BUG;
    }
                                 ...
                                 const char *not_found_err_str)
{
    ...
    if (_vector_size(vec) == 1) {
        ...
        *sim_xxx = lsm_hash_copy(*sim_xxx);
        ...
        rc = LSM_ERR_PLUGIN_BUG;
        _lsm_err_msg_set(err_msg, "Got more than 1 data with id %" PRIu64
                         "in table %s", sim_id, table_name);
        goto out;
    }
...
{
    return _db_sim_xxx_of_sim_id(err_msg, db, _DB_TABLE_FS_SNAPS_VIEW,
                                 sim_fs_snap_id,
                                 sim_fs_snap, LSM_ERR_NOT_FOUND_FS_SS,
                                 "FS snapshot not found");
}

int _db_sim_exp_of_sim_id(char *err_msg, sqlite3 *db, uint64_t sim_exp_id,
                          lsm_hash **sim_exp)
{
    return _db_sim_xxx_of_sim_id(err_msg, db, _DB_TABLE_NFS_EXPS_VIEW,
                                 sim_exp_id, sim_exp,
...
int _db_sim_disk_of_sim_id(char *err_msg, sqlite3 *db, uint64_t sim_disk_id,
                           lsm_hash **sim_disk)
{
    return _db_sim_xxx_of_sim_id(err_msg, db, _DB_TABLE_DISKS_VIEW,
                                 sim_disk_id, sim_disk, LSM_ERR_NOT_FOUND_DISK,
                                 "Disk not found");
}
                                   ...
                                   uint32_t *supported_strip_size_count)
{
    ...
    len = sizeof(uint32_t) * (sizeof(_SUPPORTED_RAID_TYPES) /
                              sizeof(_SUPPORTED_RAID_TYPES[0]));
    ...
    *supported_raid_type_count = len / sizeof(uint32_t);
    ...
    len = sizeof(uint32_t) * (sizeof(_SUPPORTED_STRIP_SIZES) /
                              sizeof(_SUPPORTED_STRIP_SIZES[0]));
