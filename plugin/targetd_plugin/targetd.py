...
class TargetdStorage(IStorageAreaNetwork, INfs):
    ...
    _ERROR_MAPPING = {
        TargetdError.VOLUME_MASKED: dict(
            ec=ErrorNumber.IS_MASKED, msg="Volume is masked to access group"
        ),
        TargetdError.EXISTS_INITIATOR: dict(
            ec=ErrorNumber.EXISTS_INITIATOR,
            msg="Initiator is already used by other access group",
        ),
        TargetdError.NAME_CONFLICT: dict(ec=ErrorNumber.NAME_CONFLICT, msg=None),
        TargetdError.INVALID_ARGUMENT: dict(ec=ErrorNumber.INVALID_ARGUMENT, msg=None),
        TargetdError.NO_FREE_HOST_LUN_ID:
        # TODO(Gris Ge): Add SYSTEM_LIMIT error into API
        dict(
            ec=ErrorNumber.PLUGIN_BUG,
            msg="System limit: targetd only allows %s LUN masked" % _MAX_H_LUN_ID,
        ),
        TargetdError.EMPTY_ACCESS_GROUP: dict(
            ec=ErrorNumber.NOT_FOUND_ACCESS_GROUP, msg="Access group not found"
        ),
    }
    ...
    def __init__(self):
        ...
        self.ca_cert_file = ""
        self.system = System(
            "targetd", "targetd storage appliance", System.STATUS_UNKNOWN, ""
        )
    ...
    def plugin_register(self, uri, password, timeout, flags=0):
        ...
        self.url = urlunsplit((self.scheme, self.host_with_port, PATH, None, None))
        ...
        if (
            "no_ssl_verify" in self.uri["parameters"]
        ...
        if "ca_cert_file" in self.uri["parameters"]:
            ...
            if not os.path.isfile(self.ca_cert_file):
                raise LsmError(
                    ...
                    "ca_cert_file URI parameter does not exist %s" % self.ca_cert_file,
                )
    ...
    def volumes(self, search_key=None, search_value=None, flags=0):
        volumes = []
        for p_name in (
            p["name"] for p in self._jsonrequest("pool_list") if p["type"] == "block"
        ):
    ...
    def _tgt_init_to_lsm(tgt_init, sys_id):
        return AccessGroup(
            "%s%s" % (TargetdStorage._FAKE_AG_PREFIX, md5(tgt_init["init_id"])),
            "N/A",
            [tgt_init["init_id"]],
            AccessGroup.INIT_TYPE_ISCSI_IQN,
            sys_id,
    ...
    def access_groups(self, search_key=None, search_value=None, flags=0):
        ...
        if self._flag_ag_support is True:
            tgt_inits = self._jsonrequest("initiator_list", {"standalone_only": True})
        else:
            tgt_inits = list(
                {"init_id": x}
                for x in set(
        ...
        rc_lsm_ags.extend(
            list(TargetdStorage._tgt_init_to_lsm(i, self.system.id) for i in tgt_inits)
        )

        if self._flag_ag_support is True:
            for tgt_ag in self._jsonrequest("access_group_list"):
                rc_lsm_ags.append(TargetdStorage._tgt_ag_to_lsm(tgt_ag, self.system.id))
    ...
    def access_group_create(self, name, init_id, init_type, system, flags=0):
        if system.id != self.system.id:
            raise LsmError(
                ErrorNumber.NOT_FOUND_SYSTEM, "System %s not found" % system.id
            )
    ...
    def access_group_initiator_add(self, access_group, init_id, init_type, flags=0):
        if init_type != AccessGroup.INIT_TYPE_ISCSI_IQN:
            raise LsmError(ErrorNumber.NO_SUPPORT, "Targetd only support iscsi")

        lsm_ag = self._lsm_ag_of_id(
            access_group.name,
            LsmError(ErrorNumber.NOT_FOUND_ACCESS_GROUP, "Access group not found"),
        )
        ...
        self._jsonrequest(
            ...
            dict(ag_name=access_group.name, init_id=init_id, init_type="iscsi"),
        )
    ...
    def access_group_delete(self, access_group, flags=0):
        ...
        self._jsonrequest("access_group_destroy", {"ag_name": access_group.name})
        return None
    ...
    def _tgt_masks(self):
        ...
        for tgt_exp in self._jsonrequest("export_list"):
            tgt_masks.append(
                {
                    "ag_id": "%s%s"
                    % (TargetdStorage._FAKE_AG_PREFIX, md5(tgt_exp["initiator_wwn"])),
    ...
    def volume_mask(self, access_group, volume, flags=0):
        ...
        tgt_masks = self._tgt_masks()
        if self._is_masked(access_group.id, volume.pool_id, volume.name, tgt_masks):
            raise LsmError(
                ErrorNumber.NO_STATE_CHANGE,
        ...
        if access_group.id.startswith(TargetdStorage._FAKE_AG_PREFIX):
            free_h_lun_ids = set(range(TargetdStorage._MAX_H_LUN_ID + 1)) - set(
                [m["h_lun_id"] for m in tgt_masks]
            )
    ...
    def access_groups_granted_to_volume(self, volume, flags=0):
        ...
        ag_ids = list(
            m["ag_id"]
            for m in tgt_masks
            if (m["vol_name"] == volume.name and m["pool_name"] == volume.pool_id)
        )
    ...
    def _get_volume(self, pool_id, volume_name):
        vol = [
            v
            for v in self._jsonrequest("vol_list", dict(pool=pool_id))
    ...
    def volume_create(self, pool, volume_name, size_bytes, provisioning, flags=0):
        if provisioning != Volume.PROVISION_DEFAULT:
            raise LsmError(ErrorNumber.INVALID_ARGUMENT, "Unsupported provisioning")
        ...
        self._jsonrequest(
            "vol_create", dict(pool=pool.id, name=volume_name, size=size_bytes)
        )
    ...
    def volume_replicate(self, pool, rep_type, volume_src, name, flags=0):
        ...
        self._jsonrequest(
            "vol_copy", dict(pool=pool_id, vol_orig=volume_src.name, vol_new=name)
        )
    ...
    @handle_errors
    def iscsi_chap_auth(
        self, init_id, in_user, in_password, out_user, out_password, flags=0
    ):
        self._jsonrequest(
            "initiator_set_auth",
            dict(
                initiator_wwn=init_id,
                in_user=in_user,
                in_pass=in_password,
                out_user=out_user,
    ...
    def fs_create(self, pool, name, size_bytes, flags=0):
        self._jsonrequest(
            "fs_create", dict(pool_name=pool.id, name=name, size_bytes=size_bytes)
        )
    ...
    def fs_clone(self, src_fs, dest_fs_name, snapshot=None, flags=0):
        ...
        self._jsonrequest(
            "fs_clone",
            dict(fs_uuid=src_fs.id, dest_fs_name=dest_fs_name, snapshot_id=ss_id),
        )
    ...
    def fs_snapshot_create(self, fs, snapshot_name, flags=0):
        ...
        self._jsonrequest(
            "fs_snapshot", dict(fs_uuid=fs.id, dest_ss_name=snapshot_name)
        )
    ...
    def fs_snapshot_delete(self, fs, snapshot, flags=0):
        self._jsonrequest(
            "fs_snapshot_delete", dict(fs_uuid=fs.id, ss_uuid=snapshot.id)
        )
    ...
    def exports(self, search_key=None, search_value=None, flags=0):
        ...
        for export in nfs_exports:
            key = export["path"] + TargetdStorage._option_string(export["options"])
        ...
        for le in list(tmp_exports.values()):
            ...
            exports.append(
                NfsExport(
                    TargetdStorage._calculate_export_md5(export["path"], options),
    ...
    def export_fs(
        ...
        if export_path is not None:
            raise LsmError(
                ErrorNumber.INVALID_ARGUMENT, "export_path required to be None"
            )
        ...
        for host in rw_list:
            ...
            self._jsonrequest(
                "nfs_export_add",
                dict(host=host, path=fs_path, export_path=None, options=tmp_opts),
            )
    ...
    def _jsonrequest(self, method, params=None, default_error_handler=True):
        data = json.dumps(
            dict(id=self.rpc_id, method=method, params=params, jsonrpc="2.0")
        )
        self.rpc_id += 1
