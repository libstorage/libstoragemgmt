...
static bool py_in_list(const char *needle, PyObject *haystack)
{
    ...
    if (!PyList_Check(haystack)) return false;
...
static PyObject *list_mount_paths(PyObject *self, PyObject *args)
{
    ...
    while (getmntent_r(f, &me, buff, _BUFF_SIZE)!=NULL) {
        ...
        if (statvfs(me.mnt_dir, &st)==-1) continue;
        if (st.f_fsid == 0) continue;

        // check for duplicates
        if (py_in_list(me.mnt_dir, result)) continue;
...
static PyMethodDef _methods[] = {
    { "get_fsid", find_fsid_bypath, METH_VARARGS, "Find Filesystem ID for given path."},
