...
from lsm.lsmcli.data_display import (DisplayData, PlugData, out,
...
def _check_positive_integer(num):
    ...
    try:
        ...
        raise ArgumentTypeError("invalid: not a positive integer value '%s'" %
                                num)
...
write_cache_policy_help = 'Write cache polices: ' + \
                          ...
                          '"write through"'
write_cache_policy_opt = dict(name="--policy",
                              metavar='<POLICY>',
...
disk_id_filter_opt = dict(name='--disk',
                          metavar='<DISK_ID>',
...
class CmdLine(object):
    ...
    def display_data(self, objects):
        ...
        DisplayData.display_data(objects,
    ...
    def _get_search_key_value(args):
        ...
        search_args = ((args.sys, 'system_id'), (args.pool, 'pool_id'),
                       (args.vol, 'volume_id'), (args.disk, 'disk_id'),
                       (args.ag, 'access_group_id'), (args.fs, 'fs_id'),
                       (args.nfs_export, 'nfs_export_id'), (args.tgt,
                                                            'tgt_port_id'))
    ...
    def list(self, args):
        ...
        if args.type == 'VOLUMES':
            ...
            if search_key == 'access_group_id':
                lsm_ag = _get_item(self.c.access_groups(),
                                   args.ag,
                                   "Access Group",
                                   raise_error=False)
                if lsm_ag:
            ...
            if search_key == 'access_group_id':
                search_key = 'id'
            if search_key == 'volume_id':
                lsm_vol = _get_item(self.c.volumes(),
                                    args.vol,
                                    "Volume",
                                    raise_error=False)
                if lsm_vol:
                  ...
                  and search_key not in AccessGroup.SUPPORTED_SEARCH_KEYS):
                raise ArgError("Search key '%s' is not supported by "
    ...
    def _add_rm_access_grp_init(self, args, op):
        ...
        if op:
            ...
            return self.c.access_group_initiator_delete(
                lsm_ag, init_id, init_type)
    ...
    def iscsi_chap(self, args):
        ...
        if init_type != AccessGroup.INIT_TYPE_ISCSI_IQN:
            raise ArgError("--init \"%s\" is not a valid iSCSI IQN" %
                           args.init)
        ...
        self.c.iscsi_chap_auth(init_id, args.in_user, self.args.in_pass,
                               self.args.out_user, self.args.out_pass)
    ...
    def fs_create(self, args):
        ...
        fs = self._wait_for_it(
            "fs-create", *self.c.fs_create(p, args.name,
    ...
    def fs_resize(self, args):
        ...
        if size == fs.total_space:
            raise LsmError(ErrorNumber.NO_STATE_CHANGE,
                           "Specified size same as current")
    ...
    def fs_clone(self, args):
        ...
        if args.backing_snapshot:
            # go get the snapshot
            ss = _get_item(self.c.fs_snapshots(src_fs), args.backing_snapshot,
                           "Snapshot")

        fs = self._wait_for_it("fs_clone",
                               *self.c.fs_clone(src_fs, args.dst_name, ss))
        self.display_data([fs])
    ...
    def file_clone(self, args):
        ...
        if self.args.backing_snapshot:
            # go get the snapshot
            ss = _get_item(self.c.fs_snapshots(fs), args.backing_snapshot,
                           "Snapshot")
        else:
            ss = None

        self._wait_for_it("fs_file_clone",
                          self.c.fs_file_clone(fs, args.src, args.dst, ss),
                          None)
    ...
    def volume_create(self, args):
        ...
        vol = self._wait_for_it(
            "volume-create",
            *self.c.volume_create(p, args.name, self._size(args.size),
                                  vol_provision_str_to_type(
                                      args.provisioning)))
    ...
    def fs_snap_create(self, args):
        ...
        ss = self._wait_for_it("snapshot-create",
                               *self.c.fs_snapshot_create(fs, args.name))
    ...
    def fs_snap_restore(self, args):
        ...
        if self.confirm_prompt(True):
            self._wait_for_it(
                'fs-snap-restore',
                self.c.fs_snapshot_restore(fs, ss, files, self.args.fileas,
                                           flag_all_files), None)
    ...
    def fs_export(self, args):
        ...
        export = self.c.export_fs(fs.id, args.exportpath, args.root_host,
                                  args.rw_host, args.ro_host, args.anonuid,
                                  args.anongid, args.auth_type, None)
    ...
    def volume_raid_create(self, args):
        ...
        if len(lsm_disks) != len(args.disk):
            raise LsmError(
                ErrorNumber.NOT_FOUND_DISK, "Disk ID %s not found" %
                ', '.join(set(args.disk) - set(d.id for d in all_lsm_disks)))

        busy_disks = [
            d.id for d in lsm_disks if not d.status & Disk.STATUS_FREE
        ]

        if len(busy_disks) >= 1:
            raise LsmError(ErrorNumber.DISK_NOT_FREE,
        ...
        self.display_data([
            _add_sd_paths(
                self.c.volume_raid_create(args.name, raid_type, lsm_disks,
                                          strip_size))
        ])
    ...
    def fs_dependants_rm(self, args):
        ...
        self._wait_for_it("fs-dependants-rm",
                          self.c.fs_child_dependency_rm(fs, args.file), None)
    ...
    def local_disk_list(self, args):
        ...
        for disk_path in LocalDisk.list():
            ...
            local_disks.append(
                LocalDiskInfo(disk_path, info_dict["vpd83"], info_dict["rpm"],
                              info_dict["link_type"], info_dict["serial_num"],
                              info_dict["led_status"], info_dict["link_speed"],
