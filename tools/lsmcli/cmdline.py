...
def parse_convert_init(init_id):
    ...
    """
    valid, converted_init_type, converted_init_id = AccessGroup.initiator_id_verify(
        init_id
    )
...
def _check_init(init_id):
    ...
    """
    valid, _, converted_init_id = AccessGroup.initiator_id_verify(init_id)
...
def _check_positive_integer(num):
    ...
    try:
        rc = long(num, 10)
        if rc < 0:
            raise ArgumentTypeError("invalid: require positive integer value '%d'" % rc)

        return rc
    except ValueError:
        raise ArgumentTypeError("invalid: not a positive integer value '%s'" % num)
...
def _valid_ip6_address(address):
    ...
    if (has_zeros and len(parts) <= 8) or len(parts) == 8:
        return all(len(x) <= 4 for x in parts) and all(
            x in allowed for x in "".join(parts)
        )
    return False
...
def _add_common_options(arg_parser, is_child=False):
    ...
    arg_parser.add_argument(
        "-v", "--version", action="version", version="%s %s" % (sys.argv[0], VERSION)
    )
...
init_id_opt = dict(
    name="--init", metavar="<INIT_ID>", help="Initiator ID", type=_check_init
)
...
nfs_export_id_filter_opt = dict(
    name="--nfs-export",
    metavar="<NFS_EXPORT_ID>",
    help="Search by NFS Export ID. Only supported for:\n" "(EXPORTS)",
)

disk_id_filter_opt = dict(
    name="--disk",
    metavar="<DISK_ID>",
...
tgt_id_filter_opt = dict(
    name="--tgt",
    metavar="<TGT_ID>",
    help="Search by target port ID.  Only supported for:\n" "(TARGET_PORTS)",
)

local_disk_path_opt = dict(name="--path", help="Local disk path", metavar="<DISK_PATH>")
...
cmds = (
    ...
    dict(
        ...
        optional=[
            dict(name="--strip-size", help="Strip size. " + size_help),
        ],
    ),
    dict(
    ...
    dict(
        ...
        args=[
            dict(vol_id_opt),
            dict(name="--size", metavar="<NEW_SIZE>", help="New size. %s" % size_help),
        ],
    ),
    dict(
    ...
    dict(
        ...
        args=[
            dict(name="--src-vol", metavar="<SRC_VOL_ID>", help="Source volume id"),
            dict(
                name="--dst-vol", metavar="<DST_VOL_ID>", help="Destination volume id"
            ),
            dict(
                name="--rep-type",
                metavar="<REP_TYPE>",
    ...
    dict(
        name="volume-mask",
        help="Grants access to an access group to a volume, " "like LUN Masking",
        args=[
            dict(vol_id_opt),
            dict(ag_id_opt),
        ],
    ...
    dict(
        ...
        args=[
            dict(name="--vol", metavar="<VOL_ID>", help="Targeted volume.\n"),
        ],
    ),
    dict(
    ...
    dict(
        ...
        args=[
            dict(name="--sys", metavar="<SYS_ID>", help="Targeted system.\n"),
            dict(
                name="--read-pct",
    ...
    dict(
        ...
        optional=[
            dict(name="--in-user", metavar="<IN_USER>", help="Inbound chap user name"),
            dict(name="--in-pass", metavar="<IN_PASS>", help="Inbound chap password"),
            dict(
                name="--out-user", metavar="<OUT_USER>", help="Outbound chap user name"
            ),
            dict(
                name="--out-pass", metavar="<OUT_PASS>", help="Outbound chap password"
            ),
        ],
    ),
    dict(
        ...
        args=[
            dict(name="--name", metavar="<FS_NAME>", help="name of the file system"),
            dict(size_opt),
            dict(pool_id_opt),
        ],
    ),
    ...
    dict(
        ...
        help="Restores a FS or specified files to " "previous snapshot state",
        args=[
            dict(snap_id_opt),
            dict(fs_id_opt),
        ],
    ...
    dict(
        ...
        args=[
            ...
            dict(
                name="--src",
                metavar="<SRC_FILE_PATH>",
                help="source file to clone (relative path)\n"
                "This is a repeatable argument.",
            ),
            dict(
                name="--dst",
                metavar="<DST_FILE_PATH>",
...
class CmdLine(object):
    ...
    def confirm_prompt(self, deleting):
        ...
        if not self.args.force:
            ...
            out(
                "Warning: You are about to do an operation that %s cause data "
    ...
    def display_data(self, objects):
        ...
        DisplayData.display_data(
            ...
            flag_human=self.args.human,
            flag_enum=self.args.enum,
            splitter=self.args.sep,
            flag_with_header=flag_with_header,
    ...
    def cli(self):
        ...
        for k, v in vars(known_args).items():
            if k.startswith(_CHILD_OPTION_DST_PREFIX):
                ...
                if (
                    getattr(known_args, root_k) is None
                    or getattr(known_args, root_k) is False
                ):
    ...
    def list(self, args):
        ...
        if args.type == "VOLUMES":
            ...
            if search_key == "access_group_id":
                lsm_ag = _get_item(
                    self.c.access_groups(), args.ag, "Access Group", raise_error=False
                )
                if lsm_ag:
                    lsm_vols = self.c.volumes_accessible_by_access_group(lsm_ag)
            elif search_key and search_key not in Volume.SUPPORTED_SEARCH_KEYS:
                raise ArgError(
                    "Search key '%s' is not supported by "
            ...
            self.display_data(self.c.pools(search_key, search_value))
        elif args.type == "FS":
            if search_key == "fs_id":
                search_key = "id"
            ...
            if search_key:
                raise ArgError(
                    "NFS client authentication type listing with "
            ...
            if search_key == "volume_id":
                lsm_vol = _get_item(
                    self.c.volumes(), args.vol, "Volume", raise_error=False
                )
                if lsm_vol:
                    ...
                    return self.display_data([])
            elif search_key and search_key not in AccessGroup.SUPPORTED_SEARCH_KEYS:
                raise ArgError(
                    "Search key '%s' is not supported by "
                    "Access Group listing" % search_key
                )
            self.display_data(self.c.access_groups(search_key, search_value))
            ...
            self.display_data(
                list(_add_sd_paths(d) for d in self.c.disks(search_key, search_value))
            )
        elif args.type == "TARGET_PORTS":
            if search_key == "tgt_port_id":
                search_key = "id"
            if search_key and search_key not in TargetPort.SUPPORTED_SEARCH_KEYS:
                raise ArgError(
                    "Search key '%s' is not supported by "
                    "target port listing" % search_key
                )
            self.display_data(self.c.target_ports(search_key, search_value))
            ...
            if search_key and search_key not in Battery.SUPPORTED_SEARCH_KEYS:
                raise ArgError(
                    "Search key '%s' is not supported by "
                    "battery listing" % search_key
                )
            self.display_data(self.c.batteries(search_key, search_value))
        else:
    ...
    def access_group_create(self, args):
        ...
        access_group = self.c.access_group_create(args.name, init_id, init_type, system)
    ...
    def _add_rm_access_grp_init(self, args, op):
        ...
        if op:
            return self.c.access_group_initiator_add(lsm_ag, init_id, init_type)
        else:
            return self.c.access_group_initiator_delete(lsm_ag, init_id, init_type)
    ...
    def iscsi_chap(self, args):
        ...
        if (
            self.args.out_user
            and self.args.out_pass
            and not (self.args.in_user and self.args.in_pass)
        ):
            raise ArgError(
                "out-user and out-password only supported if " "inbound is supplied"
            )

        self.c.iscsi_chap_auth(
            init_id,
            args.in_user,
            self.args.in_pass,
            self.args.out_user,
    ...
    def fs_create(self, args):
        ...
        fs = self._wait_for_it(
            "fs-create", *self.c.fs_create(p, args.name, self._size(args.size))
        )
    ...
    def fs_resize(self, args):
        ...
        if self.confirm_prompt(False):
            fs = self._wait_for_it("fs-resize", *self.c.fs_resize(fs, size))
            self.display_data([fs])

    # Used to clone a file system
    def fs_clone(self, args):
        src_fs = _get_item(self.c.fs(), args.src_fs, "Source File System")

        ss = None
        if args.backing_snapshot:
            # go get the snapshot
            ss = _get_item(
                self.c.fs_snapshots(src_fs), args.backing_snapshot, "Snapshot"
            )

        fs = self._wait_for_it("fs_clone", *self.c.fs_clone(src_fs, args.dst_name, ss))
        self.display_data([fs])
    ...
    def file_clone(self, args):
        ...
        if self.args.backing_snapshot:
            # go get the snapshot
            ss = _get_item(self.c.fs_snapshots(fs), args.backing_snapshot, "Snapshot")
        else:
            ss = None
    ...
    def fs_snap_create(self, args):
        # Get fs
        fs = _get_item(self.c.fs(), args.fs, "File System")
        ss = self._wait_for_it(
            "snapshot-create", *self.c.fs_snapshot_create(fs, args.name)
        )
        ...
        if self.args.file:
            ...
            if self.args.fileas:
                if len(self.args.file) != len(self.args.fileas):
                    raise ArgError("number of --file not equal to --fileas")
    ...
    def volume_delete(self, args):
        ...
        if self.confirm_prompt(True):
            self._wait_for_it("volume-delete", self.c.volume_delete(v), None)
    ...
    def fs_snap_delete(self, args):
        ...
        if self.confirm_prompt(True):
            self._wait_for_it("fs_snap_delete", self.c.fs_snapshot_delete(fs, ss), None)
    ...
    def volume_replicate(self, args):
        ...
        vol = self._wait_for_it(
            "replicate volume", *self.c.volume_replicate(p, rep_type, v, args.name)
        )
        self.display_data([_add_sd_paths(vol)])
    ...
    def volume_replicate_range(self, args):
        ...
        dst = _get_item(self.c.volumes(), args.dst_vol, "Destination Volume")
        ...
        if not len(src_starts) or not (
            len(src_starts) == len(dst_starts) == len(counts)
        ):
            raise ArgError(
                "Differing numbers of src_start, dest_start, " "and count parameters"
            )
    ...
    def fs_export(self, args):
        ...
        if len(args.rw_host) == 0 and len(args.ro_host) == 0:
    ...
    def volume_dependants_rm(self, args):
        v = _get_item(self.c.volumes(), args.vol, "Volume")
        self._wait_for_it(
            "volume-dependant-rm", self.c.volume_child_dependency_rm(v), None
        )

    def volume_raid_info(self, args):
        lsm_vol = _get_item(self.c.volumes(), args.vol, "Volume")
        self.display_data(
            [VolumeRAIDInfo(lsm_vol.id, *self.c.volume_raid_info(lsm_vol))]
        )

    def pool_member_info(self, args):
        lsm_pool = _get_item(self.c.pools(), args.pool, "Pool")
        self.display_data(
            [PoolRAIDInfo(lsm_pool.id, *self.c.pool_member_info(lsm_pool))]
        )
    ...
    def volume_raid_create(self, args):
        ...
        if len(busy_disks) >= 1:
            raise LsmError(
                ErrorNumber.DISK_NOT_FREE, "Disk %s is not free" % ", ".join(busy_disks)
            )
    ...
    def fs_dependants_rm(self, args):
        fs = _get_item(self.c.fs(), args.fs, "File System")
        self._wait_for_it(
            "fs-dependants-rm", self.c.fs_child_dependency_rm(fs, args.file), None
        )
    ...
    def is_connection_free_cmd(self):
        ...
        """
        if self.args.func.__name__.replace("_", "-") in _CONNECTION_FREE_COMMANDS:
            return True
        return False
        ...
        if self.is_connection_free_cmd():
            ...
            if cli:
                ...
                if os.getenv("LSM_DEBUG_PLUGIN"):
                    input("Attach debugger to plug-in, " "press <return> when ready...")
