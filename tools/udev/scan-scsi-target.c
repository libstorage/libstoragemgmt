 ...
 *
 * Note:  Per kernel Documentation/sysfs-rules.txt, sysfs is always mounted at
 * /sys
 */
...
int main(int argc, char **argv) {
    ...
    /*
     * Obtain the SCSI channel and ID, and construct the string to write to the
     * "scan" entry.
     */
    ...
    if ((dir_str = strstr(&devpath[target_pos + channel_pos + id_pos], "/")) ==
        NULL)
        invalid(argv, devpath);
    ...
    id_len =
        strlen(&devpath[target_pos + channel_pos + id_pos]) - strlen(dir_str);
    if (id_len < 1)
        invalid(argv, devpath);
