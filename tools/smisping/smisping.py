...
def get_cim_rps(c):
    ...
    for n in INTEROP_NAMESPACES:
        try:
            cim_rps = c.EnumerateInstances(
                "CIM_RegisteredProfile", namespace=n, localonly=False
            )
        except CIMError as e:
            if (
                e[0] == pywbem.CIM_ERR_NOT_SUPPORTED
                or e[0] == pywbem.CIM_ERR_INVALID_NAMESPACE
                or e[0] == pywbem.CIM_ERR_INVALID_CLASS
            ):
                pass
            else:
                raise
        ...
        if len(cim_rps):
            for cim_rp in cim_rps:
                if (
                    cim_rp["RegisteredOrganization"] == Uint16(11)
                    and "Array" == cim_rp["RegisteredName"]
                ):
                    return cim_rp
    return None
    ...
    try:
        try:
            conn = pywbem.WBEMConnection(
                url, (username, password), DEFAULT_NAMESPACE, no_verification=True
            )
